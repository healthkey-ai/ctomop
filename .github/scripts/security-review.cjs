...
function reviewDecision(pr, reviews, allowedReviewers, excludedReviewers = []) {
  const latest = new Map();
  for (const review of [...reviews].sort((a, b) => a.id - b.id)) {
    if (['APPROVED', 'CHANGES_REQUESTED', 'DISMISSED'].includes(review.state)) latest.set(review.user.login, review);
  }
  const excluded = new Set([pr.user.login, ...excludedReviewers]);
  const eligible = [...latest.values()].filter(review => allowedReviewers.has(review.user.login) && !excluded.has(review.user.login));
...
async function evaluate(github, owner, repo, pr) {
  ...
  const { data: commit } = await github.rest.repos.getCommit({ owner, repo, ref: pr.head.sha });
  const excluded = [commit.author?.login, commit.committer?.login].filter(Boolean);
  return reviewDecision(pr, reviews, allowed, excluded)
