...
`POST /api/webhooks/inbound/` with `Content-Type: application/json`,
`X-HealthKey-Source: ehr-labs`, and `X-HealthKey-Signature: sha256=<hex digest>`.
Compute HMAC-SHA256 over the **exact request body bytes** with the source secret.
The shared Python helper is
`patient_portal.webhooks.compute_hmac_signature(payload_bytes, secret)`.
...
an exact replay returns 200 with `duplicate: true`. Reusing an ID with different
...
transferred between organizations. `PATCH /api/webhooks/subscriptions/{id}/`
...
are allowed. DNS is revalidated for every attempt; connections are pinned to a
...
runs after commit. A rollback produces no delivery. Configure the same database
...
separate `celery -A ctomop beat --schedule=/tmp/promop-celerybeat-schedule`
...
`GET /api/webhooks/subscriptions/{id}/deliveries/` returns authorized delivery
...
Migration `patient_portal.0016` adds three tables and one uniqueness constraint;
