...
LOINC 69548-6 (genetic variant assessment) is not in the installed vocabulary,
so the mapping uses source key genomics:status with concept 0 and raw value.
"""
...
def seed_status(apps, schema_editor):
    ...
    Mapping.objects.using(using).get_or_create(
        ...
        defaults={
            ...
            'notes': 'Finding status (present/absent/indeterminate). '
                     'LOINC 69548-6 not in installed vocabulary; using source key fallback.',
        },
    )
